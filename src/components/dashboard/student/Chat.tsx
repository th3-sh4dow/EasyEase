...
import { collection, query, orderBy, limit, where, getDocs, addDoc, serverTimestamp } from 'firebase/firestore';
...
import { saveMessage, INSTITUTE_SUPPORT_ID } from '@/lib/firebase/chat';
import { formatDistanceToNow } from 'date-fns';
import { Skeleton } from '@/components/ui/skeleton';

// Helper to create a consistent chat ID between student and institute support
const getSupportChatId = (studentId: string) => {
    return [studentId, INSTITUTE_SUPPORT_ID].sort().join('--');
}
...
export function Chat() {
  ...
  useEffect(() => {
    if (user) {
        const chatId = getSupportChatId(user.uid);
  ...
  const handleSend = async () => {
    if (!input.trim() || !user || !firestore || !activeChatId) return;
    
    saveMessage(firestore, activeChatId, {
      senderId: user.uid,
      content: input.trim(),
      contentType: 'text',
    });
